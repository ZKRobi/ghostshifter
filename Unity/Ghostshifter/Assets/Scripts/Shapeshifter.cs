...
namespace UnityStandardAssets._2D
{
	public class Shapeshifter : MonoBehaviour 
	{
        ...
        private void Start () {
            ...
            body1 = transform.Find("Robot");
            body2 = transform.Find("Ghost");
        }
		...
		private void Update () { ///I'm apparently too lazy to write good code.
			if (CrossPlatformInputManager.GetButtonDown("Form1") && form != 0) {
                ...
                foreach (Collider c in body1.gameObject.GetComponents<Collider>())
                {
                    c.enabled = true;
                }
                foreach (Collider c in body2.gameObject.GetComponents<Collider>())
                {
                    c.enabled = false;
                }
